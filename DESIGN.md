...
# 多欄位金鑰（2026-08-24・lab-dev）

**需求（Benson 原話）**：「就是在鑰匙圈那邊新增兩個電影 api 的金鑰欄位，然後電影 app 就接上鑰匙圈的登入畫面，
然後金鑰以後就直接吃鑰匙圈的金鑰就好」。
背景：好雷嗎要 TMDB ＋ OMDb 兩把，後台一個 App 只有一格，所以他被要求手打 `{"tmdb":"…","omdb":"…"}`。他嫌蠢。

## 定案：欄位宣告在後台，儲存格式不動

`apps[].fields`（存在 `secrets.json`／`vault.json`）宣告這個 App 要哪幾格，
**後台 UI 幫他把幾格組成那一串 JSON**，加密的還是同一串。

三個候選裡選這個的理由：

| 方案 | 為什麼不選 |
|---|---|
| A. `keyring.json` 的條目改成多個密文 | 動到公開檔結構 ＋ `keyring-unlock.js` 的解密流程 → **三個 App 都要重新複製模組**，風險最大 |
| B. 每把金鑰各登記一個 App（`movie-library-tmdb`…） | 模組的 `CFG` 是單例，一個頁面只能 `init` 一次；而且權限、清單、藥丸全部要複製一份 |
| **C. 後台幫他組字串（採用）** | 只動後台 UI 與後台的資料檔；公開檔、模組、三個 App 一行都不用改 |

## 硬性條件

1. **沒宣告 `fields` 的 App 行為與產物完全不變**（旅途手帳、食譜本）。
   機器證明：`tools/check-fields.js` 的 B 組用 `git show main:server.js` 跑一次**改動前**的後台，
   兩份 `keyring.json` 的結構逐項比對、解出來的明文逐把比對。
2. **不准靜靜清掉他貼過的東西**：拆不開的舊字串整串放進第一格並標「格式待確認」。
3. **公開的 `keyring.json` 不含欄位宣告**（那是後台自己的事，公開檔沒有理由知道）。
4. **⭐ 後台的 HTTP API 永遠不吐明文金鑰**（即使 server 自己手上有）。這是這個系統原本就有的
   不變式，不是巧合——它是**憑證保管系統**，開一個「連得到 localhost:4620 就能撈走全部金鑰」的口
   （本機惡意程式、瀏覽器裡的其他分頁、DNS rebinding）划不來。
   - 我第一版做了一支 `GET /api/apps/:id/fieldvalues` 會回明文（為了把他手打的那串拆回兩格顯示），
     **PM 退件，正確**。已經**整支刪掉**——不是改成「有條件才給」，那種條件式的洞最後都會被繞過。
   - 替代流程：**拆解在 server 端做完、直接存回去**（原地遷移）；畫面只拿得到**每格的遮罩**；
     要改某一格就直接送新值上來（跟換金鑰同一個方向：**只進不出**）。
     沒打字的格子由 server 自己合併原值（`KF.merge`），前端從頭到尾不知道原值是什麼。
   - 紅線測試：`tools/check-fields.js` 的 **F 組**跑一輪完整流程、收集**所有** HTTP 回應
     （含 `/keyring.json`、`/vault.json`、404 的回應），用假金鑰的字串去比對，一次命中就紅。
5. **多欄位的遮罩比 `maskToken` 更狠**（`KF.maskField`）：`maskToken` 是為 GitHub PAT 設計的
   （`github_pat_11AB…` 前十幾碼是公開前綴），但這裡放的是 **TMDB 32 碼、OMDb 8 碼**這種
   「整串都是祕密」的金鑰，露前 14 碼等於露掉一半。一律最多「前 4 後 2」，8 碼以下整條遮掉。
6. **舊版後台送上來的資料沒有 `fields` 這個鍵時＝「這版不認得」，不是「他要清空」**
   （`KF.adoptFields`）。他手機上可能還快取著舊版的 `web/admin.js`，從手機存檔不該把宣告洗掉。
   真的要清空的話，新版會送 `fields: []`（有這個鍵、值是空陣列）。

## 順手修的上游 bug

`#kr-full .kr-go`／`.kr-ghost`／`.kr-danger` **沒有宣告自己的 `padding`**（還有 font-family、
line-height、appearance…），宿主只要有 `button{padding:0}` 這種 reset 就會滲進來。
**這不是權重輸了**——`#kr-full .kr-x` 是 (1,1,0)，壓得過宿主任何 `button{}`；
**是根本沒寫那個屬性，沒寫就沒有東西可以壓**。所以修法是把會被繼承／被 reset 的屬性通通寫死，
層內的選擇器約定（`#kr-full .kr-x{}`）維持不變；`.kr-chip` 之所以要自我加倍，是因為它住在層外、沒有 id 可以綁。

> ⚠️ 這次動到 `client/keyring-unlock.js`（只有 CSS）→ **三個 App 都要重新複製一份**，
> 或跑 `.github/workflows/sync-unlock.yml`。
> （提醒：那支 workflow 寫的是各 App 的 `public/` 與 `docs/`，而「好雷嗎」的模組放在 `js/`
> 而且不在 matrix 裡 → 好雷嗎那份要手動複製，或請人幫它加一行並指定路徑。）

## 待辦（下一輪，要有真瀏覽器才驗得了）

滿版層裡還有其他按鈕有**同一種曝險**（沒宣告自己的 padding／font-family／appearance，
宿主的 `button{}` reset 會滲進來）：**`.kr-tile`、`.kr-peek`、`.kr-x`、`.kr-back`、`.kr-eye`**。

這一輪刻意不動它們：`.kr-go`／`.kr-ghost`／`.kr-danger` 是 QA 連兩輪實際抓到的，
其餘只是理論曝險，而**要證明修對了需要真的瀏覽器**（`tools/check-unlock.js` 的 C 組
「三個宿主 computed style 逐項相同」才是這件事的真驗收）。
在沒有瀏覽器的機器上改它們＝改了也不知道有沒有改壞。
下一輪誰接手：先在有 Chrome 的機器上跑一次 `node tools/check-unlock.js` 拿到基準線，再動。


---

# 公開模式（2026-08-24・lab-dev）

**需求（Benson）**：電影評分那個 App「打開網址就能用，沒有登入畫面、沒有設定頁」。
它用的是 TMDB／OMDb 的**免費查詢金鑰**，不是憑證。

## 定案：`apps[]` 那一層加 `public` ＋ `plain`

```jsonc
{ "id": "movie-library", "name": "電影評分", "emoji": "🎬",
  "public": true, "plain": "{\"tmdb\":\"…\",\"omdb\":\"…\"}" }
```

- 放在 **`apps[]` 這一層**，不是掛在每個 user 底下——「不需要選人、不需要密碼」才成立。
- 欄位叫 `plain`（跟 `iv`／`cipher` 明確分開）：讀取端一眼分得出來這不是密文。
- 公開的 App **不再產生每個人的密文**：同一個值兩個來源，讀取端就要猜哪個算數。
- 多欄位宣告照舊適用（電影評分是 tmdb ＋ omdb 兩格，值還是那一串 JSON）。

## 防線是三道，不是一道

| 位置 | 擋什麼 |
|---|---|
| 前端警告框 | 白話講清楚代價（`KF.PUBLIC_WARNING`），**預設一律加密** |
| `POST /apps/:id/public`、`/token`、`/apps` | 值長得像 GitHub token 就 400（**不是只靠前端**） |
| `buildKeyring()` | 產生公開檔的**那一刻**再驗一次 → 中了就**降級成加密**（fail-safe） |

第三道才是真正承重的：值可能是「標成公開之後」才被換掉的。
降級而不是拋錯，是因為拋錯會讓他連「把公開關掉」都做不到——**失敗要往安全的方向倒，但不能把人鎖在外面**。

> ⚠️ 別把 `publicBlocked` 做成靜默：後台畫面要顯示「勾了公開但被擋下來」＋原因，
> 否則他會以為公開生效了，而 App 那邊拿不到值。

## 待辦（下一輪，要有真瀏覽器才驗得了）

滿版層裡其他按鈕（`.kr-tile`／`.kr-peek`／`.kr-x`／`.kr-back`／`.kr-eye`）的
「宿主 reset 滲漏」曝險還在，理由與做法見上一節的待辦。


---

# 開場外觀（splash，2026-08-25・lab-dev）

**需求（Benson）**：每個 App 的開場畫面（正中央一個色塊符號、下面 App 名字、再下面一句標語）
要能在鑰匙圈後台改，不用回去改每個 App 的程式碼。App 端怎麼讀是另一條線（`app-template`）的事，
這一章只講鑰匙圈這一側。

## 定案：`apps[]` 那一層加一個全部可選的 `splash`

```jsonc
{ "id": "trade-log", "name": "交易日誌", "emoji": "📦",
  "splash": { "name": "交易日誌", "glyph": "T", "bg": "#101820",
              "accent": "#3a7bd5", "ink": "#e6edf3", "tagline": "紀律比行情重要" } }
```

- **為什麼放在 `apps[]` 這一層**：開場畫面是「這個 App 長什麼樣」，**不是「誰在用它」**——
  它在使用者還沒選人、還沒解鎖之前就要畫出來了（App 一啟動就是開場）。掛在 `users[]` 底下
  就會變成「不解鎖看不到開場」，本末倒置。這跟 `public`／`plain` 是同一個判斷、同一層級：
  **不需要選人、不需要密碼**的東西住在 `apps[]`。
- **它是公開明文，而且跟密文完全無關**：不進任何 `cipher`、不碰 `kdf`／`iv`、不影響解密流程。
  讀取端拿不到就用 App 自己的預設值，永遠不會因為少了它而壞掉。
- **為什麼全部可選**：這六樣不是一組設定，是六個**各自獨立的覆寫**。他可能只想換一句標語，
  其他四樣沿用 App 原本的設計（那是 UX 線調過的）。做成「一組」就會強迫他為了改一句話
  而把整個配色也定死在鑰匙圈裡，App 端之後改版就再也蓋不動。
- **為什麼空值不寫進 JSON**：`keyring.json` 是**公開檔**，塞一堆 `""` 有兩個實害——
  ①讀取端分不出「他刻意設成空」與「他沒設」，於是 App 端的 fallback 寫不下去；
  ②整個 `splash` 物件在沒設定時會變成一坨空殼躺在每個 App 上。
  規則統一在 `KF.normSplash()`：**空字串、只有空白、看不懂的色票一律當作沒設定；六樣全空就
  連 `splash` 這個鍵都不會出現。** 後台按下存檔前也先跑一次，空值連送都不會送出去。

## 向下相容：三個方向都要顧

| 方向 | 做法 |
|---|---|
| 舊的 `keyring.json`／`secrets.json`（沒有 `splash`） | `normSplash()` 對 undefined／null／壞資料一律回 `null`；`loadSecrets()` 把空的直接 `delete`。**產出的公開檔與改動前逐項相同**（`check-splash.js` B 組拿 `git show main:server.js` 跑一次改動前的後台實際比對） |
| 手機還快取著**舊版 `web/admin.js`** | `KF.adoptSplash()`——跟 `adoptFields`／`adoptPublic` 同一條規則：**送上來的資料沒有 `splash` 這個鍵＝「這版不認得」，不是「他要清掉」** → 保留這台電腦原本的設定。真的要清掉，新版會明確送 `splash: null` |
| 舊版的**表單**（登記／改名那張表根本沒有開場欄位） | `POST /apps` 更新既有 App 時**完全不碰 `a.splash`**；換金鑰／改成員／改欄位那幾支也一樣。C 組有一條專門打這件事（把它改成會覆寫，測試立刻翻紅） |

## 兩套實作，兩邊都要做

**電腦後台（`public/`）走 server API，手機後台（`web/`）直接寫 vault、不走 server**——
這是這個專案已經吃過虧的地方（「一個 App 一把金鑰」那條規則只修了電腦端，等於沒修）。
所以這次一開始就兩邊都做：

- 電腦：App 那一列多一顆「開場外觀」→ 對話框（左邊即時縮圖、右邊表單）
- 手機：App 的 `⋯` → 「🎬 開場外觀（啟動畫面）」→ 底部 sheet（縮圖在最上面、**sticky**）
- `check-splash.js` 的 **D 組**是原始碼層級的檢查：兩邊的 `buildKeyring` 都要寫出 `splash`、
  兩邊都要有入口、`adoptPayload` 一定要用 `adoptSplash`。**只修一邊會被這一組擋下來。**

## 幾個實作上的判斷

1. **色票用 `<input type="color">`，不叫他手打 hex**；每個顏色可以個別「改回預設」
   （沒被動過的顏色就不會被寫進公開檔——這是「全部可選」的實際入口）。
   `normColor()` **只收 `#rgb`／`#rrggbb`**、一律轉小寫六碼；**帶 alpha 的 4／8 碼不收**
   （開場底色帶透明度沒有意義）。
   ⚠️ 但 `relLum()` 還是要會處理 4／8 碼（丟掉 alpha）——**第二道防線**：
   `keyring.json` 是可以手改的，而我們對外宣稱的是「不可能被調成看不見」。
   沒有那一段的話，`#ffffffff` 會被當成看不懂 → 亮度算成 0＝純黑 → 給白字 → **白底白字 1.00:1**
   （QA 在 app-template 那端抓到的，這份是同一套程式碼、同樣中招）。A13b 釘住這條。
2. **符號本身的字色不是設定項**（PM 2026-08-25 拍板）：由 `accent` 自動推導（`KF.onColor()`）。
   拿掉這個設定項的正當性**不是「自動算得準」，是「算法有下界」**——白字與深字各算一次
   **WCAG 對比度、取高的那個**，所以最差情況也有保證：**窮舉 16,777,216 色**實測
   **低於 3:1 的 0 個、最差 `#438c83` = 3.949:1**。**它不可能被調成看不見**，
   這才是可以不給他調的理由。（3:1 是 WCAG 給「大字／圖形元件」那條線；符號是 31px 粗體，用這條是對的。）
   ⭐ **近乎平手時偏白**（差距 <15% 就選白字）：`#3a7bd5` 白 4.22 / 深 4.35，差距只有 2.9%——
   算法拿 2.9% 當決定性差異並不合理，而開場底是深色、名字與標語都是淺字，
   符號一變近黑就從「發光的徽章」變成「挖空的洞」，跟其餘元素分屬兩套語言。
   ⚠️ **這條平手帶是拿「最差對比」換來的**（沒有它是 4.283:1，有它是 3.949:1）。
   要動那個 15% 之前**先跑 A13 全色域斷言**——把它放到 100%（等於永遠白字）時，
   A11／A12／A13／A13b 會一起翻紅、低於 3:1 的冒出 13,441 個（已實測）。
   > ⚠️ 第一版用的是「未做 gamma 校正的 sRGB 亮度 > 0.6」那種猜法，QA 掃出 **6.8% 的底色低於 3:1、
   > 最差 `#00d038` 只有 2.08:1**（飽和的綠／青被誤判成暗底而給白字）——**已退件改掉，別改回去**，
   > `relLum()` 裡的 gamma 校正不是裝飾。
   ⚠️ 這個函式與 `app-template` 那一端**逐字相同**，改要兩邊一起改；釘住它的是
   `check-splash.js` 的 A11／A12（QA 抓到的三個綠／青點）／**A13（全色域紅線）**，
   以及 E1b（拿亮黃 `#ffcc00` 實際量縮圖的 computed color）。
   把 gamma 那一行改回線性，A13 會立刻翻紅（已實測）。
3. **符號限一個字元，而且是在輸入框當場擋**（不是存檔才報錯）。切字用「第一個**字素**」而不是
   第一個碼位：`👨‍👩‍👧`、`👍🏽`、`❤️`、**`🇹🇼`（國旗是兩個區域指示符號湊的，切一半會變成 🇹）**
   都算**一個**字元。用碼位範圍判斷 ZWJ／變體選擇子／膚色／區域指示符號，刻意不用 `\p{M}`
   這種較新的語法。中文注音／拼音**組字中（`isComposing`）不動 `value`**，否則會把他打到一半的字打斷。
   **名字與標語的長度上限也是按字素算**（`sliceGraphemes`）：用 `slice(0,n)` 切 UTF-16 碼元
   會在尾端留下半個代理對，而那串字是要寫進**公開** `keyring.json` 的——App 端會顯示成一個 `�`。
4. **即時預覽是這張表單的主角**：他看不到效果就不會用。縮圖用 `KF.splashView()` 把沒設定的項目
   補上預覽預設值畫出來——⚠️ **那些補出來的值只給畫面看，永遠不會被存進去**（D5 就在守這件事）。
5. **「預覽」兩個字放在框外面**：放框裡（絕對定位在上緣）會跟置中的符號疊在一起，實測撞到。
6. **自由文字一樣要守門，而且用的是專案既有的 `looksSecret()`**：`splash` 是公開明文，
   這個 repo 已經有**兩次**「金鑰被貼進自由文字欄」的前科（說明、標題）。
   ⚠️ 刻意**不另外寫一套「像不像 GitHub token」**——那樣 TMDB 的 32 碼十六進位、`sk-proj-…`、
   `AIzaSy…` 全部照過，而且規則一分岔就只修得到一邊（這個 repo 吃過那個虧）。
   ⛔ 而且那句話是丟進 toast 的**純文字**，裡面不可以有 markdown（星號會原樣顯示成亂碼）。
   這個 repo 已經踩過兩次（手冊 D 段記過一次，這一輪改寫時又加了一次 `**`）——
   三條路（手機 toast／電腦 toast／server 的 400 JSON）吃的是**同一個字串**，有人順手排版就三處一起壞。
   釘住它的是 A10b（字串本體）／C10b（server 回的 JSON）／E14（手機上實際畫出來的 toast），
   把星號加回去三條會一起紅（已實測）。
   ⭐ 話要**先給出路**：他是在被擋下來、正在困惑的當下讀這句，所以開頭就是
   「中間加個空格、或改短一點就可以存了」，理由擺後面。
   （已知會被 `looksSecret()` 誤擋的正常標語：純數字日期、電話、`keep-it-simple-2026`、
   `StayHungryStayFoolish`、網域字串。中文標語 0 誤擋 → **刻意不改規則**，
   跟專案既有守門共用一套比較重要，代價由那句出路吸收。）
7. **手機端的觸控目標一律 ≥ 44×44**，而且測試是**掃描整張 sheet 的所有可互動元素**，
   不是列白名單點名（上一輪就是點名了四個，結果漏掉「全部改回 App 預設」那顆＝38px）。
   ⚠️ `#sp-reset` 是單獨拉到 44px 的：它借用 `.fr-acts` 的版面，而
   **`.fr-acts button` 在「金鑰欄位」那張 sheet 仍然是 38px —— 已知、另案**，
   這一輪刻意不動共用規則（動它會改到這輪沒驗過的畫面）。
   電腦後台（`public/admin.css`）不套這條：滑鼠不吃 44px 那個規則。

## ⚠️ 量預覽顏色的坑（下一個人會踩）

縮圖有 `transition:background .12s`。**改完顏色的當下去量 `getComputedStyle().backgroundColor`
拿到的還是舊顏色**（轉場途中回的是當下的中間值），會得出「預覽根本沒動」的相反結論——
這一輪就是這樣先紅了一次。**量之前先等 ~300ms**（`check-splash.js` E 組有註解寫明）。

## 順手發現、這一輪刻意沒動的（給下一輪）

**`web/admin.js` 的 `buildKeyring()` 沒有輸出 `public`／`plain`**（只有 `id`／`name`／`emoji`）。
也就是說**從手機後台存任何一筆，公開的 `keyring.json` 就會掉一次「好雷嗎」的公開金鑰**，
要等下次從電腦後台存檔才會補回來（`vault.json` 那份真本沒事，`public` 旗標還在）。
這是公開模式那一輪就存在的既有缺口，**不是這次改出來的**。

沒有順手修的理由：補上去要連 `publicPlain()` 的降級判斷（`publicBlockReason` → 擋到就退回加密）
一起搬到手機端，那條是**安全閘門**，這張工單的界線明寫不准碰。
下一輪要修的人：兩邊的 `buildKeyring` 應該共用同一份邏輯（搬進 `keyring-fields.js`），
不要再各寫一次——這一章上面那張「兩套實作」的表就是同一個病的第三次發作。

## 驗收怎麼跑

```
node tools/check-splash.js            # 53 條，全綠才算過
