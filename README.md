...
改成：進分頁載一次 → 按了啟動／停止之後追幾次（只在有工具是「啟動中」時，最多 4 次）→
其餘時候按「🔄 重新整理」。畫面上誠實寫「狀態是 HH:MM 抓的」。

### 改完要跑

```
node tools/check-tools.js          # 工具分頁 36 條（沙箱＋自己造的假工具，不會去開真的那幾支）
node tools/check-tools.js --only=B # A 清單搬遷／B 啟動停止／C 偵測／D 突變／E 只接受本機
```

D 組是突變測試：把「排除面板自己與子孫」那行拿掉，C2／C3 必須**翻紅**——
不然那兩條等於沒在驗。B6／C1／C4／E1／E4 是負控組（先證明尺是活的）。
