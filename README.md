...
- **偵測到但沒有 PID（純靠 port 探到的）就停不掉**，畫面要講清楚「去原本開它的視窗關」。
...
```
node tools/check-tools.js          # 工具分頁 44 條（沙箱＋自己造的假工具，不會去開真的那幾支）
node tools/check-tools.js --only=B # A 清單搬遷／B 啟動停止／C 偵測／D 突變／E 只接受本機／F 重啟／G 突變
```

D／G 兩組是突變測試：D 把「排除面板自己與子孫」那行拿掉，C2／C3 必須**翻紅**；
G 把重啟的空窗標記拿掉，F2 必須**翻紅**——不然那幾條等於沒在驗。
B6／C1／C4／E1／E4／F5 是負控組（先證明尺是活的）。

> ⚠️ 跑 `--only=` 跑到一半停掉，沙箱那支假工具可能會留下來變孤兒，
> 下次跑整套時 B 組會被它卡住（會說「已經有外部程序在跑了」）。
> 清法：把指令列含 `__tmp__faketool` 的程序 taskkill 掉。
